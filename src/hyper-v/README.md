...
The `debian-image` directory contains the `build.ps1` script and `preseed` configuration files which will create two custom images:
...
* `preseed-hvk8s.cfg` - defines an image with Docker, kubeadm and associated tools installed, this will become control plane and worker node VMs

The preseed images are created by running the build script as follows:
```pwsh
./build.ps1 preseed-dhcp-dns.cfg
./build.ps1 preseed-hvk8s.cfg
```
